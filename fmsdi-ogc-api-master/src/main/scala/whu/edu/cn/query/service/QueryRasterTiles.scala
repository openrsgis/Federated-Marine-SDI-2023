...
object QueryRasterTiles {
  ...
  /**
   * Query raster tiles serially and return a RDD of which.
   *
  ...
  /**
   * Query raster tiles and return an array of which.
   *
   * @param p Query parameter
   * @return An array of queried raster tiles.
   */
  ...
  /**
   * Initiate a RasterTile object from dimension keys.
   *
  ...
  /**
   * Generate TileLayerArray[(SpaceTimeBandKey, Tile)] with RasterTileLayerMetadata[SpaceTimeKey].
   *
   * @param rasterTiles
   * @return An Array[(SpaceTimeBandKey, Tile)] with RasterTileLayerMetadata
   */
  ...
  /**
   * Turn RasterTile to (SpaceTimeBandKey, Tile).
   *
   * @param rasterTiles
   * @return An ArrayBuffer[(SpaceTimeBandKey, Tile)]
   */
  ...
  def getPyramidTile(p: QueryParams): Array[Byte] = {
    ...
    if (conn != null) {
      try {
        ...
        val tilePngBytes = getTileCell("hbase_raster_regions", tileDataID, "rasterData", "tile")
  ...
  /**
   * Get RangeSet of a coverage.
   *
   * @param p
   * @return
   */
  def getCoverageRangeSet(p: QueryParams): Array[Double] = {
    val tileArray:(Array[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = getTiles(p)
    ...
    val stitched: Raster[Tile] = whu.edu.cn.query.util.Stitch.stitchArrayTiles(tileLayerArray, layout)
    stitched.tile.toArrayDouble()
  }


  /**
   * Return png bytes in a bounding box or subset.
   *
   * @param p Query parameters
   ...
   * @return
   */
  def getCoveragePng1(p: QueryParams, bbox: String, subset: Array[String]): Array[Byte] = {
    val tileArray:(Array[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = getTiles(p)
    val tileLayerArray: Array[(SpatialKey, Tile)] = tileArray._1.map(x=>(x._1.spaceTimeKey.spatialKey, x._2))
    val metadata = tileArray._2.tileLayerMetadata
    val layout = metadata.layout
