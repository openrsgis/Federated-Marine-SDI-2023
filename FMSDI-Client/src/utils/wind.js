 ...
 *风场类
 ****/
export function CanvasWindy(json, params) {
  //风场json数据
  this.windData = json
  //可配置参数
  ...
  this.isdestroyed = false //是否销毁，进行删除操作
  this._init()
}
CanvasWindy.prototype = {
  ...
  _init: function() {
    ...
    this.initExtent =
      this.extent.length === 0
        ? [this.windField.west - 180, this.windField.east - 180, this.windField.south, this.windField.north]
        : this.extent
  ...
  isInExtent: function(lng, lat) {
    var calcExtent = this.extent.length ? this.extent : this.initExtent
  ...
  _tomap: function(lng, lat, particle) {
    ...
    // 判断当前点是否在地球可见端
    var isVisible = new Cesium.EllipsoidalOccluder(Cesium.Ellipsoid.WGS84, this.viewer.camera.position).isPointVisible(
      ct3
    )
    ...
    // console.log(pos);
    return pos ? [pos.x - this.canvas.offsetLeft, pos.y - this.canvas.offsetTop] : null
  },
  //根据经纬度，算出棋盘格位置
  _togrid: function(lng, lat) {
    ...
    //   field.rows
    // );
    var x = ((lng - field.west) / (field.east - field.west)) * (field.cols - 1)
    var y = ((field.north - lat) / (field.north - field.south)) * (field.rows - 1)
    return [x, y]
  },
  _drawLines: function() {
    var self = this
      ...
      try {
        if (hasextent) {
          ...
          var cartesian = this.viewer.camera.pickEllipsoid(
            new Cesium.Cartesian2(this.canvas.offsetLeft + pos_x, this.canvas.offsetTop + pos_y),
 ...
 *根据风场数据生产风场棋盘网格
 ****/
export function CanvasWindField(obj) {
  this.west = null
  this.east = null
  this.south = null
  this.north = null
