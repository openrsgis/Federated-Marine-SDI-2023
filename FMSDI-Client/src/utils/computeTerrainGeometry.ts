...
function computeTerrainGeometry(
  rect: Cesium.Rectangle,
  sampler: (lng: number, lat: number) => number,
  options: {
    latSamples?: number
    lngSamples?: number
    NO_DATA_VALUE?: number
    replaceNoData?: false | number
    exaggerationMultiplier?: number
    colorSteps?: {
      height: number
      color: ColorTuple
    }[] // in height ascending order, color represented by RBGA, for example, [255,255,255,1.0]
  } = {
    latSamples: 10,
    lngSamples: 10,
  ...
  const { latSamples, lngSamples, colorSteps, NO_DATA_VALUE, replaceNoData, exaggerationMultiplier } = options

  const computedColorSteps = colorSteps.map(({ color, height }) => ({
    color: [color[0] / 255.0, color[1] / 255.0, color[2] / 255.0, color[3]] as ColorTuple,
    height
  }))
  ...
  function interpolateColor(height: number): [number, number, number, number] {
    ...
    while (i < computedColorSteps.length) {
      if (height < computedColorSteps[i].height) {
        ...
        return Array(4)
          .fill(0.0)
          .map((_, idx) => hgtScale * (rclr[idx] - lclr[idx]) + lclr[idx]) as any
      }
      ++i
    }
  }
  ...
  for (let gridY = 0; gridY < latSamples; gridY++) {
    for (let gridX = 0; gridX < lngSamples; gridX++) {
      ...
      const replacedHeight = height === NO_DATA_VALUE && replaceNoData !== false ? replaceNoData : height
      const exaggeratedHeight = replacedHeight * exaggerationMultiplier
      // console.log(
      //     `height at (${lng},${lat}): ${height}>${replacedHeight}`
      // );
